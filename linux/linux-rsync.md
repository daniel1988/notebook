...
## 远程文件同步到本地
rsync -av 192.168.1.111::php/crm.csv ./
...
```
rsync -av root@127.0.0.1::www /data
```

    5)从本地机器拷贝文件到远程rsync服务器中。当DST路径信息包含"::"分隔符时启动该模式。如：
```
rsync -av /data root@127.0.0.1::www
```

    6)列远程机的文件列表。这类似于rsync传输，不过只要在命令中省略掉本地机信息即可。如：
```
rsync -v rsync://127.0.0.1/www
```
...
## rsync参数的具体解释如下：
    ...
    >ps:一般都使用azv选项（如果windows与linux同步不建议使用这个参数）